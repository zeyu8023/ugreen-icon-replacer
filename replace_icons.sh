#!/bin/bash

# 日志文件保存路径
LOG_FILE="/var/log/icon_replace_$(date +%Y%m%d_%H%M%S).log"

# 获取脚本所在路径（兼容 curl 流式执行等情况）
if [[ -n "${BASH_SOURCE[0]}" && "${BASH_SOURCE[0]}" != "$0" ]]; then
  BASE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
else
  BASE_DIR="$(pwd)"
fi

# 提示用户输入图标目录路径（支持默认值）
read -p "请输入图标目录路径（留空则使用当前目录下的 icons 文件夹）: " USER_INPUT

# 确定图标路径来源
if [ -z "$USER_INPUT" ]; then
  ICON_SOURCE_DIR="$BASE_DIR/icons"
  echo "🧭 使用默认图标目录：$ICON_SOURCE_DIR" | tee -a "$LOG_FILE"
else
  ICON_SOURCE_DIR="$USER_INPUT"
  echo "📁 使用用户输入目录：$ICON_SOURCE_DIR" | tee -a "$LOG_FILE"
fi

# 检查路径有效性
if [ ! -d "$ICON_SOURCE_DIR" ]; then
  echo "❌ 找不到目录：$ICON_SOURCE_DIR" | tee -a "$LOG_FILE"
  exit 1
fi

# 确认是否继续
read -p "确认将图标替换 /ugreen/static/icons 吗？操作不可撤销。(y/n): " CONFIRM
if [[ "$CONFIRM" != "y" && "$CONFIRM" != "Y" ]]; then
  echo "🚫 已取消操作。" | tee -a "$LOG_FILE"
  exit 0
fi

# 开始执行替换并记录日志
{
echo "🕒 替换开始：$(date)"
echo "📁 图标来源目录：$ICON_SOURCE_DIR"
echo "🎯 替换目标路径：/ugreen/static/icons"

find "$ICON_SOURCE_DIR" -type f \( -iname "*.png" -o -iname "*.svg" -o -iname "*.jpg" \) | while read -r file; do
  filename=$(basename "$file")
  target="/ugreen/static/icons/$filename"

  if [ -f "$target" ]; then
    cp "$file" "$target"
    echo "✅ 替换成功：$filename"
  else
    echo "⚠️ 未找到对应系统图标，跳过：$filename"
  fi
done

echo "✅ 全部图标替换完成：$(date)"
} | tee -a "$LOG_FILE"
